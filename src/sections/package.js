...
export default function Package({ showModal, onSelectPackage, formData }) {
  return (
    <section id="pricing" sx={styles.pricing} className=" pt-16">
      <Container>
        <SectionHeader
          title="AIMOOV Lancement exclusif"
          slogan="Choisissez votre offre"
          isWhite={true}
        />
        <Flex
          sx={{
        ...
        >
          {packages.map((packageData) => (
            <PriceCard data={packageData} key={packageData.name} showModal={showModal} onSelect={onSelectPackage} formData={formData} />
          ))}
        </Flex>
      </Container>
    </section>
...
const PatternBG = "images/patternBG.png";
const styles = {
  pricing: {
